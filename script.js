...
/* Hero slideshow: create slides, preload images, crossfade every 4s */
(function(){
  ...
  var fadeMs = 1000; // match CSS
  var prefersReduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

  // Preload
  var pre = images.map(function(src){ var i=new Image(); i.src=src; return i; });

  // Insert slides before overlay
  var overlay = hero.querySelector('.hero-overlay');
  var slides = images.map(function(src){
    var d = document.createElement('div'); d.className='hero-slide'; d.style.backgroundImage='url("'+src+'")';
    if(overlay) hero.insertBefore(d, overlay); else hero.appendChild(d);
    return d;
  });

  var current = 0, timer = null;
  function showInitial(){ slides.forEach(function(s){s.classList.remove('is-active')}); slides[0].classList.add('is-active'); }
  if(prefersReduced){ showInitial(); return; }

  function next(){ var n=(current+1)%slides.length; slides[n].classList.add('is-active'); setTimeout(function(){ slides[current].classList.remove('is-active'); current=n; }, fadeMs+30); }
  function start(){ showInitial(); timer = setInterval(next, interval); }

  // start after a small delay or when first image loads
  var startTimeout = setTimeout(start, 250);
  if(pre[0] && pre[0].complete){ clearTimeout(startTimeout); start(); } else if(pre[0]) pre[0].onload=function(){ clearTimeout(startTimeout); start(); };

  document.addEventListener('visibilitychange', function(){ if(document.hidden){ if(timer){ clearInterval(timer); timer=null; }} else { if(!timer) timer=setInterval(next, interval); } });
